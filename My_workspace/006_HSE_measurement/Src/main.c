...
#define RCC_CR					(RCC_BASE_ADDRESS)
...
int main(void)
{
	...
	//1. Enable the HSE clock using RCC_CR
	*pRCC_CR |= (1 << 16);

	//2. Wait until HSE clock from external crystal is ready
	while(!(*pRCC_CR & (1 << 17)));
	...
	*pRCC_CFGR |= 1;
	...
	//2. Select clock source for MCO1 is HSE
	*pRCC_CFGR |= (1 << 22);
	//	 Config MCO1 prescaler is 4
	*pRCC_CFGR |= (1 << 25);
	*pRCC_CFGR |= (1 << 26);
